"""CAD-focused runtime tool registrations.

Extracted from runtime_tool_registry.py so the registry file stops growing
monolithically and CAD-specific handlers live next to the cad_generation module
they call into. Handlers remain closures over ``active_settings`` (and the
feature-parameter loader) so the registration surface is unchanged.
"""
...
def register_cad_tools(rt: Any, active_settings: Any, load_project_feature_parameters: Any) -> None:
    """Register all cad.* runtime tools."""
    ...
    def _tool_cad_list_editable_parameters(inp: dict[str, Any], _ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        index = load_project_feature_parameters(str(project_id))
        if index is None:
            return {
                "status": "ok",
